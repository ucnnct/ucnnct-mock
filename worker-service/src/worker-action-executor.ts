...
import { SessionObjective, UserAction } from './models.js';
import {
  baseLatencyForAction,
  errorChanceForAction,
  requestCostForAction
} from './worker-action-profile.js';
import {
  ActionOutcome,
  VirtualUserState,
  ...
  applyAction(
    ...
    switch (action) {
      ...
      case 'open_home':
        ...
        this.services.scheduleLiveTraffic(assignment, user, action);
        return this.outcome(requestCost, latencyMs, 'Navigated back to the home feed.');
      ...
      case 'logout':
        ...
        return this.outcome(requestCost, latencyMs, 'Closed the session and entered offline cooldown.');
    }
  }
